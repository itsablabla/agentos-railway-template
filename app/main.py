...
# ---------------------------------------------------------------------------
# API endpoint to expose available models
# ---------------------------------------------------------------------------
@app.get("/api/models")
async def get_available_models():
    """Return list of available LLM models from custom provider and defaults."""
    custom_api_key = getenv("CUSTOM_LLM_API_KEY")
    custom_base_url = getenv("CUSTOM_LLM_BASE_URL")
    
    models_data = []
    
    # Add custom LLM models if configured
    if custom_api_key and custom_base_url:
        model_categories = {
            "claude": getenv("LLM_MODELS_CLAUDE", ""),
            "gpt": getenv("LLM_MODELS_GPT", ""),
                ...
                for model_id in model_list.split(","):
                    ...
                    if model_id:
                        models_data.append({
                            "id": model_id,
                            "object": "model",
                            "owned_by": f"custom-{category}",
                            "provider": "custom-llm"
                        })
    
    # Add default OpenAI models
    default_models = ["gpt-4o", "gpt-4o-mini", "gpt-4.1", "gpt-4.1-mini", "o3", "o4-mini"]
